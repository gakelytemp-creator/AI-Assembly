...
12. [Perplexity — exact model/version not exposed to participant](RESPONSES/12_PERPLEXITY_MODEL_UNDISCLOSED.md) — recorded; [verification attempt 1](VERIFICATIONS/12_PERPLEXITY_ATTEMPT_1_UNVERIFIABLE.md) returned **UNVERIFIABLE** because the participant interface could not fetch the repository URL. The participant explicitly confirmed that standalone `svg` strings and fenced `text` markers were rendering artifacts; those were removed. Final raw-text fidelity verification remains pending.
